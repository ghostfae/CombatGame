...
{
   public static Spell SelectSpell(UnitState unit) 
   {
   ...
   public static UnitState SelectEnemy(IEnumerable<UnitState> availableTargets, UnitState self)
   {
      var enemies = availableTargets.Where(t => t.Side != self.Side).ToArray();
      return enemies[Rng.Random.Next(enemies.Length)];
   }

   public static UnitState SelectAlly(IEnumerable<UnitState> availableTargets, UnitState self)
   {
